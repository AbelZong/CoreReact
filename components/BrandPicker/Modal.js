...
export default React.createClass({
  ...
  handleOK() {
    ...
    const valueName = value ? this.state.dataList.filter(x => x.ID === value)[0].Name : ''
            ...
            <div className={styles.dataList}>
              <ScrollerBar autoHide>
                <RadioGroup onChange={this.handleRadio} value={this.state.value}>
                  {dataList.map((x) => <Radio key={x.ID} value={x.ID}><span title={x.Intro}>{x.Name}</span></Radio>)}
                </RadioGroup>
              </ScrollerBar>
            </div>
          ) : null}
