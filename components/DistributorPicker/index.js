...
export default React.createClass({
  ...
  render() {
    ...
    const styler = style ? {width: this.props.width || 112, ...style} : {width: this.props.width || 112}
    return (
      <div className={CN} style={styler}>
        <div className={styles.inputArea} onClick={this.handleSelect}>
          <Input value={this.state.name} placeholder='选择供销商' size={this.props.size || 'default'} className={styles.input} />
          <span className={styles.operator}>
            {this.state.name !== '' ? <Icon type='minus' onClick={this.handleRemove} title='点击移除' /> : <Icon type='ellipsis-h' title='点击选择' />}
          </span>
        </div>
        <Modal hideClear={this.props.hideClear} visible={this.state.visible} onOk={this.handleModalOk} onCancel={this.handleModalCancel} value={this.state.value} />
      </div>
    )
  }
})
