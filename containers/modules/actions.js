...
//notReDefault 路由不存在，是否不重定向到默认
export function setCurrentBookmark(notReDefault) {
  return (dispatch, getState) => {
    const {bookmarks, bookmarkAIndex} = getState()
    let bookmark = bookmarks[bookmarkAIndex]
    if (!bookmark && bookmarkAIndex !== 0 && !notReDefault) {
    ...
    if (bookmark) {
      ...
      console.log('current router not exists! %s', bookmarkAIndex)
    }
  }
}
